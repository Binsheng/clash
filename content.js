...
function processText(text) {
  return text.replace(/_{4,}/g, '  "blank"  ');
}


var abc = []; // 将 abc 数组设置为全局变量
...
if (CN_CURRENT_MESSAGE && CN_CURRENT_MESSAGE.length) {
	...
	if (newSentences != null && newSentences.length != CN_CURRENT_MESSAGE_SENTENCES.length) {
            ...
            var collectedLastParts = []; // 初始化收集 lastPart 的数组

            for (i = nextRead; i < newSentences.length; i++) {
                CN_CURRENT_MESSAGE_SENTENCES_NEXT_READ = i+1;

                var lastPart = newSentences[i];
                collectedLastParts.push(lastPart); // 将 lastPart 添加到数组中
            }

            CN_CURRENT_MESSAGE_SENTENCES = newSentences;

            if (collectedLastParts.length > 0) { // 确保数组中有元素
                var abc = collectedLastParts.join(' '); // 将收集到的 lastPart 数组转换为以空格分隔的字符串
                var def = processText(abc);
                CN_SayOutLoud(def); // 使用 CN_SayOutLoud 函数读出字符串
            }
        }
    }

    setTimeout(CN_CheckNewMessages, 2500);
}
...
function CN_InitScript() {
	...
	jQuery("body").append("<span style='position: fixed; top: 8px; right: 16px; display: inline-block; " +
		...
		"font-weight: bold; z-index: 1111;' id='TTGPTSettings'><a href='https://github.com/C-Nedelcu/talk-to-chatgpt' target=_blank title='Visit project website'>ChatGPT</a><br />" +
